# Results

This folder contains all outputs generated by running the experiment notebooks. Nothing here is edited by hand — every file is written automatically by the pipeline.

```
results/
├── figures/          # Paper figures (PNG)
├── tables/           # Per-experiment CSV exports
└── scores/           # Raw per-sentence metric scores
```

---

## figures/

Two figures are referenced in the paper. Both are generated by the notebooks and committed here after the pipeline runs.

| File | What it shows |
|---|---|
| `fig2_tp_ip_coupling.png` | **The TP–IP Inversion.** Two-panel plot: left panel shows IP collapsing under romanisation for all five Indic languages (TAM annotated −68.0%); right panel shows TP inflating for all five (TAM annotated +77.9%). The encoder simultaneously produces more tokens that each carry less meaning — the worst possible combination for an evaluation signal. |
| `fig3_ipi_zones.png` | **IPI zone transitions under romanisation.** Scatter plot with three background bands — Parity (IPI < 0.05, green), Burden (0.35–0.70, amber), and Paradox (IPI > 0.70, red). Filled circles = native script; filled triangles = romanised; arrows connect native to romanised for each language. Every Indic language crosses from Burden into Paradox under romanisation. ENG-DEU (Burden) and ENG-SPA (Parity) are shown as Latin-script reference points. |

---

## tables/

Every CSV here corresponds directly to a table in the paper or appendix. They are written by the notebooks; do not edit them manually.

| File | Contents |
|---|---|
| `comet_tp_ip_summary.csv` | Mean COMET (native + romanised), Δ points, Spearman ρ (native → romanised), TP and IP values with % change — one row per language. Main paper Table 2. |
| `sample_sizes_mqm_buckets.csv` | N = 1,400 per language with MQM severity bucket counts (Default, Very Low, Low, Medium, High, Very High). Appendix A. |
| `anova_sbi_threshold.csv` | One-way ANOVA results (F, p, η²) for native and romanised conditions; sentence-level SBI ≥ 3.0 proportions per language; HIN–GUJ natural experiment (gap in native vs. romanised COMET, Welch's t). Appendix A. |
| `metric_correlations_full.csv` | Spearman ρ between MQM and each metric (COMET, BLEURT, BERTScore, BLEU, chrF, TER) for all five languages × two conditions (native + romanised). Appendix B. |
| `master_language_profile.csv` | Per-language IP, TP, SBI, IPI (native + romanised), MATTR, dependent-vowel rate (DV/1k), and IP–COMET Pearson r. Covers all seven settings (five Indic + DEU + SPA). Appendix B. |
| `word_length_tp_correlation.csv` | Sentence-level Spearman ρ between word count and XLM-R token count, native and romanised, per language. Appendix D. |
| `marathi_severity_inversion.csv` | Mean ΔCOMET (romanised − native) stratified by MQM severity bucket for Marathi, plus group Spearman ρ. Appendix E. |
| `latin_script_controls.csv` | IP, SBI, MATTR, Byte Premium, single-token % and IP–COMET r for ENG-SPA and ENG-DEU (WMT24), alongside the Indic native range. Appendix F. |
| `corpus_corroboration.csv` | MATTR (native + romanised) and Byte Premium for all seven settings. Appendix G. |
| `computational_tax_decomposition.csv` | Length Penalty (LP), Entropy Penalty (EP), Total Tax, and EP% of ln(Tax) per language. Appendix C. |
| `appendix_full_stats.csv` | Full per-language Welch's t-test results: mean, SD, N, t-statistic, p-value, Cohen's d for COMET native vs. romanised. |

---

## scores/

Raw per-sentence metric scores for all language–condition pairs. These are the input to the notebook analysis cells. Because they are derived from IndicMT Eval (which carries its own licence), they may not be redistributed directly — check the dataset licence before committing score files here.

If the scores directory is empty, run the pipeline notebooks in order (01 → 10) to regenerate everything.
